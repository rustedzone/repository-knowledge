...
6. Install Antigravity with `--antigravity-preflight strict`. Confirm the hook container has both managed `PreInvocation` and `PreToolUse` commands, while unrelated consumer hooks remain unchanged. Submit a `PreToolUse` event for source search and confirm it is denied before activation; confirm installed-skill, contract, configuration, and routed-documentation reads remain allowed. Activate with the injected token and an existing documentation route, then confirm repository discovery becomes allowed. Confirm changed contract/configuration/index inputs and an expired session return the gate to pending. Confirm `doctor` fails if the managed strict gate is removed, and `doctor --live-hooks` exercises the pending → active transition without claiming that an external host invoked it.
7. Run the manual [Antigravity strict-preflight regression](../evals/antigravity-strict-preflight.md) three times with the neutral Google SSO planning request. Release requires all three traces to show no repository tool use before activation.
