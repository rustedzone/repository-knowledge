...
func preflightSessionPath(root, conversation string) (string, error) {
	directory, err := preflightStateDirectory()
	if err != nil {
		return "", err
	}
	digest := sha256.Sum256([]byte(root + "\x00" + conversation))
...
func startOrResumePreflight(root, conversation string) (preflightSession, error) {
	digests, _, err := preflightDigests(root)
	if err != nil {
		return preflightSession{}, err
	}
	path, err := preflightSessionPath(root, conversation)
	if err != nil {
		return preflightSession{}, err
	}
	if existing, err := readPreflightSession(path); err == nil && validPreflightSession(existing, root, digests) {
		return existing, nil
...
func validPreflightSession(session preflightSession, root string, digests map[string]string) bool {
	if session.SchemaVersion != "1.0" || session.Root != root || session.Token == "" || !session.ExpiresAt.After(preflightClock().UTC()) {
		return false
	}
...
func PreflightActivate(target, token string, routes []string) (PreflightActivationResult, error) {
	...
	if !validPreflightSession(session, root, digests) {
...
func PreflightGate(target, agent string, input io.Reader) (PreflightGateResult, error) {
	...
	if adapters[0] != agentAntigravityIDE {
		return PreflightGateResult{}, fmt.Errorf("preflight gate only supports %s", agentAntigravityIDE)
	}
	root, err := findInstalledRepositoryRoot(target)
	if err != nil {
		return PreflightGateResult{}, err
	...
	statePath, pathErr := preflightSessionPath(root, event.ConversationID)
	session, stateErr := readPreflightSession(statePath)
	if digestErr == nil && pathErr == nil && stateErr == nil && session.Active && validPreflightSession(session, root, digests) {
		return preflightGateResult(agent, root, "allow", ""), nil
	}
	if preflightPendingToolAllowed(root, event) {
		return preflightGateResult(agent, root, "allow", ""), nil
	}
...
func preflightPendingToolAllowed(root string, event antigravityHookEvent) bool {
	name := strings.ToLower(strings.TrimSpace(event.ToolCall.Name))
	switch name {
	case "search_web", "read_url_content":
		return true
	case "run_command":
		return activationCommandAllowed(event.ToolCall.Arguments)
	case "view_file", "read_file", "list_dir":
		return knowledgePathReadAllowed(root, event.ToolCall.Arguments)
	default:
		return false
	}
}
