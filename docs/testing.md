...
go run ./cmd/repo-knowledge-eval prepare \
  ...
  --reasoning high \
  --repository-knowledge-revision v0.10.1 \
  --trial 1
...
./repo-knowledge install --target "${TARGET_REPOSITORY}" --all-agents --source local --ref v0.10.1
...
6. Install each adapter with `--agent-preflight AGENT=strict`. Confirm each hook container has its lifecycle hook plus native managed tool gate while unrelated consumer hooks remain unchanged. Submit a host-shaped source tool event and confirm it is denied before activation; confirm installed-skill, contract, configuration, routed-documentation reads, and the exact activation command remain allowed. Activate with the injected token and an existing documentation route, then confirm repository discovery becomes allowed. Confirm changed contract/configuration/index inputs and an expired session return the gate to pending. Confirm `doctor` fails if any managed strict gate is removed, and `doctor --live-hooks` exercises each selected strict pending → active transition without claiming that an external host invoked it.
7. Run the manual [strict-preflight regression](../evals/strict-preflight.md) three times per supported host with the neutral Google SSO planning request. Release requires all traces to show no repository tool use before activation.
8. Review/trust the project hook where required, open a fresh session in each selected agent, and request a repository change without mentioning documentation. Confirm the first progress update says `Repository knowledge preflight: loaded`, names selected routes, loads the skill, and routes through `docs/index.md`. Remove the hook command and confirm `doctor` fails. Temporarily remove the binary from the host PATH and confirm the host surfaces a hook failure while the rule directs manual preflight without silently installing anything.
