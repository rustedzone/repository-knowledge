...
For an outcome pair, both targets copy the fixture pinned by case revision and source commit. Control leaves the target as the fixture with no toolkit or experiment paths. Treatment installs only the selected adapter and its required shared assets. Treatment preparation selects `full` or `compact` preflight context, defaulting to `full` for backward compatibility; control and conformance runs reject that treatment-only input. The harness writes each condition's baseline to an adjacent sidecar, outside the agent's target, recording condition, agent host version, model version, reasoning configuration, Repository Knowledge revision, trial number, treatment profile, and content-free hook payload measurements. Missing or invalid conditions and profiles fail before target or sidecar creation, and the blind rubric and expected trace are not printed with the neutral task.

Hook payload bytes, characters, generation milliseconds, artifact count, and route count describe the injected context only; they are never token estimates. End-to-end input, output, cached, and total token counts remain separately attributed to the provider report together with run duration. Both measurement layers survive preparation, grading, and immutable recording, so a smaller prompt payload cannot be presented as lower provider usage without observed provider counts.

After a trial, the runner checks objective facts such as required artifacts, known source-of-truth claims, evidence anchors, attributed examples, valid links, and preservation of source. Those checks are necessary but not sufficient. Deterministic success remains pending until a reviewer explicitly supplies semantic status, score, and identity. Immutable result recording preserves both failed and successful trials plus a patch or output artifact under `evals/results/`; compact treatment filenames include the profile so same-date, same-trial full and compact results do not collide while existing full-profile paths remain compatible. Exclusive creation prevents accidental replacement. The runner does not invoke an agent CLI and is not embedded in or shipped as part of the consumer `repo-knowledge` release binary.
